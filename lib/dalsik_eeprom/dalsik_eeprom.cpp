...
KeyInfo EEPROM::get_key(uint8_t layer, KeyCoords c) {
    int address  = get_eeprom_address(layer, c);
    uint8_t type = read(address + 0x00);
    uint8_t key  = read(address + 0x01);

    return KeyInfo(type, key, c);
}

KeyInfo EEPROM::get_tapdance_key(uint8_t index, uint8_t tap) {
...
int EEPROM::get_eeprom_address(uint8_t layer, KeyCoords c) {
    return sizeof(EEPROM_KeyInfo)*(
        layer*KEY_COUNT + c.row*2*ONE_SIDE_COL_PIN_COUNT + c.col
    );
}

int EEPROM::get_tapdance_eeprom_address(uint8_t index, uint8_t tap) {
