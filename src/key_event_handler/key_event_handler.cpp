...
KeyEventHandler::KeyEventHandler():
    ...
    combos_handler(&this->tapdance_handler),
    keyboard_side(EEPROM::get_keyboard_side())
{
}

void KeyEventHandler::send_slave_event_to_master(BasicKeyEvent event) {
...
// We use 1B to send type, row and col from BasicKeyEvent + 2 parity
// ABBCCCDE
...
uint8_t KeyEventHandler::encode_slave_event(BasicKeyEvent event) {
    uint8_t data = 0x00;
...
BasicKeyEvent KeyEventHandler::decode_slave_event(uint8_t data) {
    BasicKeyEvent event;
    ...
    if (p1 == calc_p1 && p2 == calc_p2) {
        ...
        return BasicKeyEvent { EVENT_NONE, KeyCoords { 0, 0 } };
    }
}
...
void KeyEventHandler::handle_key_event_from_master(BasicKeyEvent event, millisec now) {
...
void KeyEventHandler::handle_key_event_from_slave(uint8_t data, millisec now) {
    BasicKeyEvent event = this->decode_slave_event(data);
...
void KeyEventHandler::handle_timeout(millisec now) {
    this->handle_key_event(BasicKeyEvent { EVENT_TIMEOUT, COORDS_INVALID }, now);
}

void KeyEventHandler::handle_key_event(BasicKeyEvent event, millisec now) {
#if DEBUG
    if (event.type != EVENT_TIMEOUT) {
        Serial.print("BasicKeyEvent <");
        if (event.type == EVENT_KEY_PRESS) {
            Serial.print("PRE");
        } else {
            Serial.print("REL");
    ...
    KeyEvent key_event = KeyEvent(event, now);

    this->combos_handler.handle_key_event(key_event);
}

void KeyEventHandler::print_internal_state(millisec now) {
    this->combos_handler.print_internal_state(now);
